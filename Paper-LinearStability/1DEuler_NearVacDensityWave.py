...
Note: can save and load data with save_results_compressed and load_results_compressed functions.
...
savefile = None #'1dEuler_densitywave_near0_circulant_n39'

perturbation = 0 # if 0, use Floquet mode, If 1, use random noise on density. If 2, use random noise on all variables. 
...
# ---- Plot 1.6: H-norm of perturbation v_E(t)
plt.figure(figsize=(5, 4))
for case_label, data in results.items():
    plt.semilogy(
        data["t"][:data["tmax_idx"]],
...
ax.set_ylim(ymin, ymax)
plt.xlabel(r"$t$", fontsize=16, labelpad=-5)
plt.ylabel(r"$\| \boldsymbol{e}_{\rho} \|_{\mathsf{H}}$", fontsize=16, labelpad=22, rotation=0)
plt.tick_params(axis="both", labelsize=13)
plt.legend(fontsize=12, loc="best")
plt.grid(True, alpha=0.3)
plt.tight_layout()
