...
savefile = 'Euler2dVortex_CSBPp43eDiv' # will add extension + .png automatically
tm_method = 'rk4'
cfl = 0.1
...
# trad: 3, p2 elem: [20,40,80,160], p3 elem: [15,30,60,120], p4 elem: [12,24,48,96]
...
if op == 'csbp' or op == 'hgtl':
    ...
    if p == 1: sig = 0.05
    elif p == 2: sig = 0.00125
    elif p == 3: sig = 0.00075
    elif p == 4: sig = 0.000015
...
if disc_type == 'div':
    ...
    if include_upwind:
        #TODO: how to handle upwind for element type?? Use gassner?
        schedule1 = [['disc_nodes',opu],['nen',*nen],['p',int(2*pu), int(2*pu+1)],
                    ['vol_diss',{'diss_type':'upwind', 'fluxvec':'dt', 'coeff':1.}],
                    ['disc_type','div']]
        labels1 = [f'Upwind $p_\\text{{u}}={int(2*pu)}$', f'Upwind $p_\\text{{u}}={int(2*pu+1)}$']
    else:
        schedule1 = []
        labels1 = []
    ...
    schedule3 = [['disc_nodes',op],['nen',*nen],['p',p],['disc_type','div'],
    ...
    if include_upwind:
        #TODO: how to handle upwind for element type?? Use gassner?
        schedule1 = [['disc_nodes','upwind'],['nen',*nen],['p',int(2*pu), int(2*pu+1)],
                    ['vol_diss',{'diss_type':'upwind', 'fluxvec':'dt', 'coeff':1.}],
                    ['disc_type','div']]
        labels1 = [f'Upwind $p_\\text{{u}}={int(2*pu)}$', f'Upwind $p_\\text{{u}}={int(2*pu+1)}$']
    else:
    ...
    if include_bothdiss:
        schedule2 = [['disc_nodes',op],['nen',*nen],['p',p],['disc_type','had'],
                    ['vol_diss',{'diss_type':'dcp', 'jac_type':'scalar', 's':s, 'bdy_fix':True, 'use_H':True, 'coeff':0.2*sig},
                                {'diss_type':'dcp', 'jac_type':'matrix', 's':s, 'bdy_fix':True, 'use_H':True, 'coeff':0.2*sig}]]
        labels2 = [f'Cons. Sca. $\\sigma={0.2*sig:g}$', f'Cons. Mat. $\\sigma={0.2*sig:g}$']

    schedule3 = [['disc_nodes',op],['nen',*nen],['p',p],['disc_type','had'],
                ['vol_diss',{'diss_type':'nd'},
...
if loaddata:
    ...
    solver = PdeSolverSbp(diffeq, settings, tm_method, dt, tf,
                        ...
                        nelem=nelem[0], nen=nen[0], disc_nodes='csbp',
