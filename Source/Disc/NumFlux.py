...
class NumFlux:
    ...
    def init_numflux_class(self, method):
        '''
        Purpose
        ----------
        ...
        assert self.dim == 1, 'Num Flux only for 1D are available'
        ...
        if method == 'central':
            ...
            self.numflux = lambda qL,qR: self.LF_scalar(qL,qR,1,C)
        elif method == 'split':
            if self.diffeq.diffeq_name == 'Burgers':
                self.numflux = lambda qL,qR: self.split(qL,qR,2/3)
            else:
                print('ERROR: split flux not set up for this Diffeq')
        elif method == 'tadmor':
            if self.diffeq.diffeq_name == 'Burgers':
                self.numflux = lambda qL,qR: self.tadmor(qL,qR,2/3)
        elif method == 'rusanov':
            if self.neq_node == 1:
